...
class Overall:
    ...
    def total_funding_mom(self):
        ...
        """
        temp_df = startup.groupby(['year', 'month'])[
            'amount'].sum().reset_index()
        temp_df['MM-YYYY'] = temp_df['month'].astype(
    ...
    def most_funded_cities(self):
        ...
        most_funded_city.loc[most_funded_city['city'] == 'Bangalore', 'amount'] += \
            most_funded_city.loc[most_funded_city['city']
                                 == 'Bengaluru', 'amount'].values[0]

        # Drop the Bengaluru row
        most_funded_city = most_funded_city[most_funded_city['city']
                                            != 'Bengaluru']
    ...
    def top_investors(self):
        ...
        top_investors = new_df.groupby(
            'investors')['amount'].sum().reset_index()
        ...
        top_investors.loc[top_investors['investors'] ==
                          'SoftBank Group', 'amount'] = updated_amount
    ...
    def funding_amount_year_month(self):
        ...
        df_agg = startup.groupby(['year', 'month'])[
            'amount'].sum().reset_index()

        # Create pivot table
        pivot_table = df_agg.pivot(
    ...
    def calculate_funding_by_month_and_year(self):
        ...
        Returns:
            pandas.DataFrame: Pivot table containing the funding amounts for each month and year.
        """
        # Aggregate funding amount by year and month
        df_agg = startup.groupby(['year', 'month'])[
            'amount'].sum().reset_index()
